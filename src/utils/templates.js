...
export const genBrandAd = async (brandImg, title, description) => {
  const template = await Jimp.read(
  ...
  const productImgX = templateW / 2 - productImgContainerW / 2; // centering the product image horizontally
  const productImgY = templateH - (productImgContainerH + templateH * (1 / 20)); // product image at a height of 10% from template base
